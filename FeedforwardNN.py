...
class NeuralNetwork():
    ...
    def back_propagate(self, output, target):
        ...
        for layer in reversed(range(2, self.num_layers)):  # All layers except input/output
            ...
            deltas[layer] = np.multiply(np.dot(weights, prev_deltas), self.sigmoid_derivative(a_val))
    ...
    def train(self, inputs, targets, num_epochs, learning_rate=1, stop_accuracy=1e-4):
        error = []
...
if __name__ == "__main__":
    ...
    #Predict it with some data for testing
